...
DB_HOST = "localhost"
DB_USER = "api_user"
DB_PASSWORD = os.environ.get("DB_PASSWORD")  # Store password in environment variable
DB_NAME = "wifi_data"
...
def create_ssid():
    ...
    if not ssid:
        return jsonify({'error': 'SSID is required'}), 400  # Bad Request

    connection = get_db_connection()
    if not connection:
        return jsonify({'error': 'Database connection failed'}), 500  # Internal Server Error

    cursor = connection.cursor()
    try:
        cursor.execute("INSERT INTO ssids (ssid) VALUES (%s)", (ssid,))
        connection.commit()
        new_ssid_id = cursor.lastrowid  # Get the ID of the newly inserted row
        cursor.close()
        connection.close()
        return jsonify({'message': 'SSID created successfully', 'id': new_ssid_id}), 201  # Created
    except mysql.connector.Error as err:
        print(f"Error inserting SSID: {err}")
        connection.rollback()
        cursor.close()
        connection.close()
        return jsonify({'error': 'Failed to create SSID'}), 500  # Internal Server Error
